 ...
 * 批量删除面试记录（仅限超级管理员）
 * 
 * @param interviewIds 要删除的面试ID数组
 * @returns 删除结果
 */
export async function bulkDeleteInterviews(interviewIds: string[]): Promise<{
  success: boolean
  message?: string
  deletedCount?: number
  deletedInterviews?: Array<{
    id: string
    student_email: string | null
    student_name: string | null
  }>
  ...
  try {
    console.log("[Bulk Delete] Deleting interviews:", interviewIds)
    ...
    if (!response.ok) {
      ...
      return {
        success: false,
        error: result.error || 'Failed to delete interviews'
      }
    }
